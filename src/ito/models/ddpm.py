...
from torch_scatter import scatter
from tqdm import tqdm
from hydra.utils import get_class  # <-- add this
...
class DDPMBase(pl.LightningModule):
    def __init__(
        self,
        score_model_class,               # str | type[nn.Module]
        ...
        **kwargs,
        ...
        # 1) Lightning init MUST NOT receive extra kwargs
        super().__init__()

        # 2) Save hyperparams safely (avoid saving raw class object)
        #    If a string was provided, save it; if not, save the path-ish repr.
        class_name = (
            score_model_class
            if isinstance(score_model_class, str)
            else f"{score_model_class.__module__}.{score_model_class.__name__}"
        )
        self.save_hyperparameters(
            {
                "score_model_class": class_name,
                "score_model_kwargs": dict(score_model_kwargs or {}),
        ...
        # 7) EMA on score model params
        self.ema = ema.ExponentialMovingAverage(
            self.score_model.parameters(), decay=0.99
        )

    def forward(self, *args):
        return self.score_model(*args)
    ...
    def configure_optimizers(self):
        ...
        return {
            "optimizer": optimizer,
            "lr_scheduler": {"scheduler": lr_scheduler, "monitor": "train/loss"},
        }

    def _ode_sample(self, batch, forward_callback, ode_steps=100):
        ns = dpm_solve.NoiseScheduleVP("discrete", betas=self.betas)
...
class TLDDPM(DDPMBase):
    def sample(self, cond_batch, ode_steps=0):
        cond_batch = cond_batch.to(self.device)  # safer to reassign
    ...
    def get_loss(self, batch):
        ...
        noise_batch, epsilon = self.get_noise_img_and_epsilon(batch_t)
        epsilon_hat = self.forward(noise_batch, batch_0)
        loss = nn.functional.mse_loss(epsilon_hat.x, epsilon.x, reduction="none").sum(-1)
        loss = scatter(loss, noise_batch.batch, reduce="mean").mean()
        return loss
