...
#             embedding.NominalEmbedding("atom_number", n_features, n_types=n_types),
            embedding.MLPEmbedding(feature_name="node_feat", in_dim=4, hidden_dim=2*n_features, out_dim=n_features),
...
#         print(batch_0)
        ...
        # Test to check how the model performs with randomized inv and eqv features
#         noise_batch.invariant_node_features = torch.zeros_like(embedded.invariant_node_features)
#         noise_batch.equivariant_node_features = torch.zeros_like(embedded.equivariant_node_features)
        noise_batch.edge_index = cond_edge_index
        
#         print(noise_batch, batch_0)
#         dx = noise_batch.equivariant_node_features.squeeze()
        dx = self.net(noise_batch).equivariant_node_features.squeeze()
        noise_batch.x = noise_batch.x + dx
#         noise_batch.x = dx # Cpainn tweaked
...
class Message(torch.nn.Module):
    def __init__(
        ...
        self.eq_MLP = embedding.MLP(n_features, n_features, n_features)
...
#         cross_produts = multiply_first_dim(dst_node_edges, dst_equivariant_node_features)
        ...
        # Test to see how breaking equivariance affects model performance
#         dv = self.eq_MLP(dv.transpose(1,2)).transpose(1,2)
...
#         print('batch equivariant node features shape after messsage', batch.equivariant_node_features.shape)
...
#         print('batch equivariant node features shape after update', batch.equivariant_node_features.shape)
...
#         print('batch equivariant node features shape after readout', batch.equivariant_node_features.shape)
