...
class SoftOneHotEmbedding(InvariantFeatures):
    def __init__(self, feature_name, n_features, max_radius, basis="cosine"):
        super().__init__(feature_name)
        self.embedding = SoftOneHotEncoder(n_rbf=n_features, max_radius=max_radius, basis=basis)
...
class MLPEmbedding(InvariantFeatures):
    def __init__(self, feature_name="node_feat", in_dim=None, hidden_dim=None, out_dim=None, skip_connection=False):
        super().__init__(feature_name)

        # Store dimensions for MLP
        self.in_dim = in_dim
        self.hidden_dim = hidden_dim if hidden_dim is not None else 2 * in_dim
        self.out_dim = out_dim if out_dim is not None else in_dim

        self.embedding = MLP(
            f_in=self.in_dim,
            f_hidden=self.hidden_dim,
            f_out=self.out_dim,
