...
class TemporallyLinearInterpolant(LinearInterpolant):
    ...
    def summarize_cfg(self):
        ...
        """
        super().summarize_cfg()
        print(f"[{self.__class__.__name__}] alpha=1 - t, beta=t, gamma=sqrt(2*t*(1 - t))")
...
class TrigonometricInterpolant(LinearInterpolant):
    ...
    def summarize_cfg(self):
        ...
        """
        super().summarize_cfg()
        print(f"[{self.__class__.__name__}] alpha=torch.cos(torch.pi * t / 2.0), beta=torch.sin(torch.pi * t / 2.0), gamma=torch.sqrt(2*t*(1 - t))")
...
class EncoderDecoderInterpolant(LinearInterpolant):
    ...
    def summarize_cfg(self):
        ...
        f"alpha=torch.cos(torch.pi * (t - {self._switch_time}*t)**{self._power} / "
        f"(({self._switch_time} - {self._switch_time}*t)**{self._power} + (t - {self._switch_time}*t)**{self._power}))**2 * 1_[0, {self._switch_time}), "
        f"beta=torch.cos(torch.pi * (t - {self._switch_time}*t)**{self._power} / "
        f"(({self._switch_time} - {self._switch_time}*t)**{self._power} + (t - {self._switch_time}*t)**{self._power}))**2 * 1_(1 - {self._switch_time}, 1], "
        f"gamma={self._sqrt_a} * torch.sin(torch.pi * (t - {self._switch_time}*t)**{self._power} / "
        f"(({self._switch_time} - {self._switch_time}*t)**{self._power} + (t - {self._switch_time}*t)**{self._power}))**2"
        )
...
class MirrorInterpolant(LinearInterpolant):
    ...
    def summarize_cfg(self):
        ...
        """
        super().summarize_cfg()
        print(f"[{self.__class__.__name__}] alpha=0, beta=1, gamma=torch.sqrt(2*t*(1 - t))")
